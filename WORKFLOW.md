# Freebuff 小说生成器 · 操作手册

本文件是**执行 Agent 的总纲**。所有阶段严格按此推进，禁止跳步、禁止凭对话记忆写正文。

## 一、核心原则（防「AI 记性差」）

1. **一切落盘**：所有设定、进度、总结都写入 `state/` 文件，正文写入 `novel/`。
2. **只信文件**：写任何内容前，先读取对应状态文件；**不依赖对话历史记忆**。
3. **一章一总结**：每写完一章立刻更新所有状态文件，再写下一章。
4. **断点续写**：任何时刻被打断，只需读 `state/progress.md` + `state/chapter-log.md` 即可恢复。
5. **固定提示词**：每个阶段对应 `prompts/` 下一个固定模板，换主题只改 `state/theme.txt`，流程不变。

## 二、正文反 AI 痕迹检查（写作与返修必做）

每章落稿后，除篇幅和设定检查外，必须进行一次人工语感复检：

1. 扫描“不是 A，是 B”“没有 A，只是 B”“不是为了 A，而是为了 B”等否定矫正句式；全章原则上不超过 1 次，反转改用直接动作和结果。
2. 扫描“三寸、一寸、半寸、半步”等量词；同章不超过 2 次，距离尽量交给呼吸、衣料、兵刃声音、骨节受力和脚下触感表现。阮孤灯不得套用三刀流程。
3. 按人物声线复读对白：柳九娘务实粗粝，陶未明公门干瘪，明照质朴困惑，阿七油滑松弛，梅照影冷静克制；删去全员格言腔。
4. 每章补查 2–3 处身体与环境的生理细节，并确认它们改变了耐力、判断、动作或情绪，而非装饰。
5. 检查阿七的解围方式是否与前章重复；必须轮换市井急智，禁止茶盘、水桶和改水流的固定闭环。
6. 返修第 1–11 章时，先做句式与对白脱毒，再做情节连贯性和 UTF-8 字符统计；禁止用未经人工复核的全局替换。
7. 第16章后做结构审计：每章记录主要空间、昼夜/天气、路程后果和传播压力；连续两章不得只是换房机关。阮孤灯、明照、陶未明的独立破局和阿七的真实失手必须在章节日志中登记。第1–16章还要检查火、水、烟是否跨场景遗留，人物是否被灾变拆成互不完全信任的小组。
8. 第16章后审计裴九重的行动目标：不得再用“维持自尽现场”解释被火攻、决堤和多人受伤后的所有克制；必须写出夺证、分化、伪验单、悬赏或控制传播的具体收益。
9. 每章阶段总结必须记录：本章哪个高手独立破局、哪个人物主动结盟/背叛、灾变遗留到哪里、阿七是否付出不可逆代价。
10. **禁绝清单式排比**：严禁把人物观察写成“看到了三件事/三个细节：第一、第二、第三”。观察必须随行动、触碰、对话和误判自然出现。
11. **正文与后台分离**：`novel/*.md` 只保留小说正文，禁止出现“第一幕/第二幕”、篇幅表、写作要点回顾、因果种子和作者说明；这些内容统一写入 `state/`。
12. **对戏优先**：每章主角与他人的实质互动（交易、试探、争执、协作、欺骗、救援）原则上不少于正文篇幅的50%。单人行走和环境凝视不得代替戏剧冲突。
13. **人物绑定**：新角色出场后必须在同卷内再次产生关系后果，不得完成递线索、打一架或交道具后立即消失。尽量安排不同阵营人物同场，使利益冲突承担扩写功能。
14. **中层配角优先于大人物扩张**：前两卷不再随意增加主角级或宗师级人物；优先强化半面、严平、段成和一名退役老斥候，让他们通过日常摩擦、议价、抗命、误判和照护承担戏份。
15. **配角必须有自己的账**：配角不能只为主角提供线索、道具或情绪。每名重要配角进入场景时，必须带着独立的生存目标、利益、恐惧或秘密；其选择要改变粮食、路线、证物、伤势、信任或传播结果。人物离场时须留下未结清的债、误会、损失、承诺或后续追索。
16. **对白落地**：禁止所有人物共享哲理格言、拆字机锋和“看与被看”的回环。对白优先写钱、粮、伤、时辰、家人、差役、路引和眼前危险；一句话必须服务于交易、威胁、遮掩、求饶、争执或决定。角色的阶级、职业和当下情绪决定其措辞。
17. **拒绝跑图副本**：连续章节不得反复采用“进入场所—扫描细节—取得道具—听警告—离开”的推进模式。至少一名配角必须与主角持续同行或反复纠缠；同一场景尽量容纳不同阵营的同时目标，让人物互相阻碍，而非轮流出场。
18. **历史大事写过程**：潼关、马嵬坡、长安沦陷等事件不得只写成消息切片或大事记。须通过城门、军粮、车马、伤病、搜捕、物价、逃难者和错误军令表现时间与制度如何具体失效；重大事件之间保留合理的路程和军报延迟。

## 二、正文反 AI 痕迹检查（补充：去机锋、去排班、去报时器）

### 提示词工程执行原则（2026-08-28）

纯粹的否定指令不足以约束生成结果。写作与润色提示词必须同时提供：

- 禁用结构的明确清单；
- 错误示例与《天阙》正确示例的对照；
- 禁用行为的替代动作、阻力和后果；
- 角色阶级/职业对应的对白声线；
- 落稿前可逐项勾选的自查表。

后续修改提示词时不得只增加“不要写……”；必须说明“应改写成什么、由谁承担、产生什么后果”。


10. 对白复检必须标出拆字回环和共同机锋腔；“X是什么？X就是X”“信里有字”“人有名字，脚没有”类句式若不承担威胁、交易或人物选择，直接改写。
11. 混战复检不能按角色动作排队点名；检查是否有遮挡、撞位、慢半拍、误伤器物或顾不上证物的失序后果。
12. 第1–15章扫描木鱼、敲击和“咚”字；木鱼只能保留为一次具体机关/道具动作，删除机械三声报时，改用石灰、风门、铁链、火油、梁木、药味和脚下触感推进。
13. 检查“没有A却B”“不是A反而B”“里面不是A是B”等隐蔽否定变体，不能只搜固定的“不是A，而是B”。
14. 检查阿七是否连续使用茶盘、水桶、水盆或门板精准解围；至少一处应改成市井耍赖、判断失误、挨打、求饶或证物损毁，且不可立即恢复。

## 三、阶段总览

| 阶段 | 提示词 | 输出文件 | 目的 |
|------|--------|----------|------|
| 前置 | 用户口述 | `state/requirements.md` | 细化并固化用户对这本书的全部要求（总约束） |
| 00 | `prompts/00-theme.md` | `state/theme.md` | 定题材、基调、视角、规模 |
| 01 | `prompts/01-outline.md` | `state/outline.md` | 立大纲（三幕/起承转合） |
| 02 | `prompts/02-characters.md` | `state/characters.md` | 列人物：性格/目的/秘密/弧线 |
| 03 | `prompts/03-conflicts.md` | `state/conflicts.md` | 列矛盾关系矩阵 |
| 04 | `prompts/04-chapters.md` | `state/chapters.md` | 列章节规划 |
| 05 | `prompts/05-foreshadowing.md` | `state/foreshadowing.md` | 列伏笔暗线 |
| 06 | `prompts/06-timeline.md` | `state/timeline.md` | 列时间线 |
| 07 | `prompts/07-knowledge.md` | `state/knowledge.md` | 人物在时间线上知道/不知道/错误认知 |
| 08 | `prompts/08-plot-check.md` | `state/plot-check.md` | 全盘漏洞检查（动笔前必过） |
| 09 | `prompts/09-write-chapter.md` | `novel/chapter-N.md` | 写一章正文 |
| 10 | `prompts/10-summary.md` | `state/chapter-log.md` 等 | 章节总结 + 状态同步 |
| 11 | `prompts/11-continue.md` | — | 循环 09→10 直到写完 |
| 12 | `prompts/12-polish.md` | `novel/*.md` | 润色与衔接 |
| 13 | `prompts/13-final-proof.md` | `state/final-check.md` | 精校交付 |

## 三、主循环（阶段 09 → 10 → 11）

```
while 当前章节 < 总章节数:
    执行 阶段09（写一章）
    执行 阶段10（总结 + 同步全部状态文件）
    当前章节 += 1
```

**禁止**：连续写多章再补总结；跳过状态同步；凭记忆写下一章。

## 四、断点恢复（换会话 / 被打断时）

1. 读取 `state/progress.md`。
2. 若「当前阶段」≤ 08：从对应阶段提示词继续。
3. 若「当前阶段」= 09/10：读取 `state/chapter-log.md` 最后一篇，确认当前章节，从阶段 09 继续。
4. 若「当前阶段」= 12/13：从对应阶段继续。
5. 恢复后**先检查**：`state/foreshadowing.md`、`state/knowledge.md`、`state/timeline.md` 是否与已写章节一致，不一致先修正再继续。

## 五、修订记录（改设定的唯一合法途径）

任何设定改动（人物、大纲、章节、伏笔）必须：

1. 在 `state/revisions.md` 追加一条：日期 / 改了什么 / 为什么 / 影响哪些章节。
2. 同步修改对应状态文件。
3. 在 `state/plot-check.md` 记录一次局部复检。
4. **禁止**在写正文时悄悄改设定。

## 六、状态文件清单

| 文件 | 内容 | 何时更新 |
|------|------|----------|
| `state/theme.txt` | 用户填的主题（唯一需要用户改的文件） | 用户 |
| `state/requirements.md` | 用户要求的细化与固化（全书总约束，高于一切） | 开书时 |
| `state/theme.md` | 题材/基调/视角/规模 | 阶段 00 |
| `state/outline.md` | 大纲 | 阶段 01 |
| `state/characters.md` | 人物 + 状态备注 | 阶段 02 / 每章后 |
| `state/artifacts.md` | 道具/法宝设定与伏笔 | 阶段 02 / 新道具出现时 |
| `state/conflicts.md` | 矛盾矩阵 | 阶段 03 |
| `state/chapters.md` | 章节规划 | 阶段 04 |
| `state/foreshadowing.md` | 伏笔暗线表 | 阶段 05 / 每章后 |
| `state/timeline.md` | 时间线 | 阶段 06 / 每章后 |
| `state/knowledge.md` | 知识矩阵 | 阶段 07 / 每章后 |
| `state/plot-check.md` | 漏洞检查报告 | 阶段 08 / 修订时 |
| `state/chapter-log.md` | 每章总结（防记忆丢失核心） | 每章后 |
| `state/progress.md` | 当前阶段 / 当前章节 / 总章节数 | 每阶段后 |
| `state/revisions.md` | 修订记录 | 任何设定改动时 |
| `state/final-check.md` | 精校报告 | 阶段 13 |

## 七、进度文件格式（`state/progress.md`）

```markdown
# 进度

- 当前阶段：09
- 当前章节：1
- 总章节数：20
- 状态：写作中
- 上次更新时间：YYYY-MM-DD
```

「状态」取值：初始化 / 规划中 / 写作中 / 润色中 / 已完成。
