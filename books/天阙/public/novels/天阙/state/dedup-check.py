...
比对:  ① 冻结短语(F类, dedup-baseline-vol3.md) 逐字匹配
       ...
       ⑨ F10 章题声明与旧题禁引 (自 ch141 起): 新章题必须先声明于 outline-vol7.md;
...
# 弃用旧题(从 chapter-title-index.md 解析): 卷六旧题, 卷七起正文禁现(指代/概念语可豁免需登记)
DEPRECATED = ["粮行的一笔", "船坞的三十年", "岸上的袁五", "南支的老人", "虹县的图", "出关的册",
              "马市的编号", "地窖的药案", "寺的供养册", "雪线之前", "军线的回响"]
# 弃用旧题中允许以人物/地点指代词在正文出现的豁免项 (防误伤, 仍禁止作为章题引用)
DEPRECATED_EXEMPT_BODY = {"岸上的袁五"}
...
def main():
    ...
    for n in args:
        ...
        if n < 141:
            ...
            decl_all = load_outline_declared("outline-vol7.md")
            if not decl_all:
                f10_hits.append("outline-vol7.md 缺失或未声明任何章题——ch141+ 章题必须先声明于 state/outline-vol7.md（声明章题后再跑查重）")
            elif n not in decl_all:
                f10_hits.append(f"第{n}章未在 outline-vol7.md 中声明——先补声明（`第{n}章《…》`）再跑查重")
